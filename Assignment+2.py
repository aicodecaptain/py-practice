...
def answer_three():
    x= (df['Gold']/(df['Gold'].sum() +df['Gold.1'].sum())).abs()-(df['Gold.1']/(df['Gold'].sum() +df['Gold.1'].sum())).abs()
    return x.idxmax()
...
def answer_four():
    G= lambda G:df['Gold']*3+df['Gold.1']*3+df['Gold.2']*3+df['Silver']*2+df['Silver.1']*2+df['Silver.2']*2+df['Bronze']*1+df['Bronze.1']*1+df['Bronze.2']*1 
    return G
